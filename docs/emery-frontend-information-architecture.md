# Emery Frontend Information Architecture

## Purpose

This document starts Phase 4 against the real supervisor backend after `Emery-15`.
...
The finer-grained activity model described in `Emery-9` is not yet fully driven by the runtime. `activity_state` is not yet enough to power confident `actively_working` versus `waiting_for_input` UX on its own.

### Thin Client Rule

Before `Emery-9` lands, the shell should use a two-level presentation:
...
Without that, the client can show live/not-live correctly, but not the full minimal animated working-state model described in `Emery-9`.
...
- session state indicators are driven by backend runtime state, not tab hemerystics
