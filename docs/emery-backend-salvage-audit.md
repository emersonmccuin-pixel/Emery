# Emery Backend Salvage Audit

## Purpose

This document audits the current Forge Rust backend as input to the Emery rebuild. The goal is not to stabilize Forge in place. The goal is to identify which backend ideas and modules are worth carrying forward into the supervisor-backed Emery architecture and which ones should be rewritten or dropped.

Locked Emery assumptions applied during this audit:
...
- Emery-native docs are canonical DB-backed docs
- agents use MCP and/or CLI tools, never raw DB access
- raw transcripts stay in artifact storage, not the knowledge graph

## Executive Summary

Forge is not a viable codebase to continue evolving directly into Emery. It contains valuable subsystem knowledge and some reusable logic, but the backend shape is still wrong for the locked architecture.
...
- the operational schema is too session-centric and too narrow for Emery's multi-root project, planning, and restore model
- the knowledge store mixes useful ideas with Forge-specific WCP import baggage and does not match the new Emery graph model
...
- this aligns with Emery's rule that agents stay opaque interactive programs
...
- generated branch naming is directionally aligned with Emery
...
- extraction provenance and retry behavior should influence Emery's knowledge admission pipeline
...
- the actual IPC mechanism is wrong for Emery: it uses command/response JSON files in directories plus polling
...
- artifact path conventions are useful and should shape Emery's session artifact layout
- the naming is Forge-specific and still couples path concerns to Tauri app state
- `forge.db` and `engram.db` do not match the Emery naming and boundary decisions
...
- redesign the tool surface against Emery application services
- add matching CLI access with the same service layer underneath

### Rewrite

These subsystems are too mismatched with the locked Emery model to preserve structurally.
...
- statuses, item types, and entity kinds do not align with the Emery architecture docs
- canonical docs and extracted knowledge provenance need a cleaner Emery-native graph model

Emery replacement:

- new `knowledge.db`
- Emery-native graph schema for work items, docs, session nodes, extracted entries, links, embeddings, and optional external refs
...
- current state names do not match Emery's locked crash and restore semantics
...
- this is the opposite of the locked Emery separation between reconnecting client and durable supervisor
...
- Emery needs a cleaner split between session telemetry, activity model, and presentation read models

Emery replacement:

- preserve specific parsing hemerystics where they are still useful
- rewrite around supervisor events and normalized artifact readers

### Discard

These should not be carried into Emery.
...
- they do not belong in the clean Emery architecture
...
- Emery has a better architectural framing: admission gating first, librarian only if justified later
...
| Worktree creation and cleanup | Keep | Good starting point for Emery git layer |
...
| `forge.db` schema | Rewrite | Too narrow and wrong boundary for Emery |
...
| Metadata/activity/title logic | Rewrite | Preserve hemerystics selectively |
| Backup service | Rewrite | Requirements changed around Emery packaging and backup scope |
| Raw terminal experiments | Discard | Not part of Emery v1 |
...
Do not continue the Forge backend as the Emery backend.
