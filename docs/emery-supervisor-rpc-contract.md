# Emery Durable Supervisor And Local RPC Contract

## Purpose

This document defines the Emery v1 durable supervisor and the local RPC contract between the reconnecting Tauri client and the per-user Rust supervisor.
...
Emery v1 has these process classes:

- `emery-client` as the Tauri desktop shell
- `emery-supervisor` as the per-user Rust background daemon
...
- `\\\\.\\pipe\\emery-supervisor-<instance-key>`
...
- `crates/emery-cli-tools`
- `apps/emery-client-tauri`
