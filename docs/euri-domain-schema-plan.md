# EURI Domain Model And Schema Plan

## Purpose

This document defines the concrete EURI v1 domain model and storage plan across:
...
- EURI-native docs are canonical DB-backed docs in `knowledge.db`
...
- use text UUID IDs in new EURI schema for both databases
...
| `last_used_at` | INTEGER NULL | useful for cleanup heuristics |
| `created_at` | INTEGER NOT NULL | epoch ms |
| `updated_at` | INTEGER NOT NULL | epoch ms |
...
Canonical EURI-native authored documents.
...
EURI should adopt a clean dual-database design:
