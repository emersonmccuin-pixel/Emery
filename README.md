...
Emery is a supervisor-first system. A durable Rust daemon owns all sessions, worktrees, and state. The UI is a Tauri/React shell that connects over local IPC. If the UI crashes, sessions keep running. Restart and reconnect.

```
apps/
  emery-supervisor/    Tauri host + supervisor daemon entry point
  emery-client/        React frontend (xterm.js terminal, work panels, fleet view)
...
The core UX loop — and what makes Emery more than a terminal multiplexer — is the hybrid dispatch model.
...
2. **Dispatcher** — the Emery UI, which creates isolated environments and launches agents into them
3. **Integrator** — the Emery merge queue, where the user reviews diffs, orders merges, and resolves conflicts
...
4. Emery creates worktree branches, seeds context bundles, launches agents
...
powershell -ExecutionPolicy Bypass -File .\scripts\start-emery-client-hidden.ps1

# With diagnostics
powershell -ExecutionPolicy Bypass -File .\scripts\start-emery-client-hidden.ps1 -EnableDiagnostics

# Stop
powershell -ExecutionPolicy Bypass -File .\scripts\stop-emery-client-hidden.ps1
```
...
Set `EMERY_DEV_DIAGNOSTICS=1` to enable structured JSONL event logging:
