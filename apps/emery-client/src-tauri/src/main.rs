...
fn debug_dev_server_preflight() -> Result<()> {
    ...
    Err(anyhow!(
        "Tauri debug startup requires the frontend dev server at http://{endpoint}. Start the client with `cargo tauri dev` or run the Vite dev server on port 1420 before launching `emery-client`."
    ))
}
