...
pub fn tool_merge_queue_get_diff() -> Value {
    json!({
        "name": "euri_merge_queue_get_diff",
...
pub fn tool_merge_queue_merge() -> Value {
    json!({
        "name": "euri_merge_queue_merge",
        "description": "Merge a ready merge queue entry into the target branch. Entry must have status 'ready' — run euri_merge_queue_check first if unsure.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "entry_id": { "type": "string", "description": "Merge queue entry ID" }
...
pub fn handle_merge_queue_merge(input: Value) -> Result<String> {
    ...
    if status != "ready" {
        return Err(anyhow!(
            "Entry {} has status '{}', not 'ready'. Run euri_merge_queue_check first to resolve conflicts and mark it ready.",
            entry_id,
            status
        ));
    }
